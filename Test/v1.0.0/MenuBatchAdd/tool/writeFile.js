...
- 三级菜单： 最普遍
- 四级菜单： 也可以
- 三级菜单下面还有子菜单但是不显示仍然作为三级菜单，也能实现目录文件创建(但是有点问题就是还需要手动调整)
- 

2. type = 2 并且 main为true(表示主文件) 创建最后一层目录下的 所有 index文件
3. type = 2 unify:true创建 insert,find,update 文件
...
function createListAndIndex(paths, type) {
    ...
    currentPaths.forEach((item, index) => {
        ...
        if (index !== 0 || type !== 0) {
            let path_permi_index = path.join(permi_dir,
                path_list.replace(eval('/' + item + '$/'), '') + config.main + config.js),
                path_rule_index = path.join(rule_dir,
                    path_list.replace(eval('/' + item + '$/'), '') + config.main + config.js),
...
2. 主文件中又有主文件，当然这个是同级的关系，因为菜单下级不能有菜单
