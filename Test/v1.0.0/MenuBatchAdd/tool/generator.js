 ...
 * @property orderNum: 3 * index + 1 根据索引自动生成  3n+1
 *  */
const testConfig = [
    {
        type: 0,
...
function type2(item, index, paths) {
    ...
    return {
        ...
        orderNum: 3 * index + 1, //排序
        perms: paths.join(':').replace(/\//g, ':') + ':page', // 权限标识
        component: paths.join('/'),
        name: paths.join('_').replace(/\//g, '_'),// 菜单name
        children: item.children && generator(item.children, { paths }),
        ...mixins,
    }
}
...
function type3(item, index, paths) {
    ...
    return {
        ...
        menuName: item.menuName, // 菜单名称
        orderNum: 3 * index + 1, //排序
        perms: paths.join(':').replace(/\//g, ':'), // 权限标识
    }
}
